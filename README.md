...
3. **Accept / Reject** — each suggestion can be rejected (with a 5-second **Undo**) or accepted. **Accepting generates a tailored CV from the job URL — in the posting's language, reusing the posting already read during the scan (no re-fetch) — and takes you straight to the CV Generator**, which shows the URL it's building from.
