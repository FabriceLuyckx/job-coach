...
Offer a single, user-triggered generic application — an untargeted CV and
cover-letter guide aimed at the profile's target roles instead of one posting —
pinned atop the Applications page, generated from a role brief synthesized from
the profile with no network fetch, and behaving like any other application once
created.
## Requirements
