...
Host the packaged application in its own native window instead of a browser
tab, so the running app owns its Dock/taskbar identity and closing the window
is a real quit — without changing how the app is reached in development or on
a platform with no usable web view runtime.
